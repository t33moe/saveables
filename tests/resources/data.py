...
from src.contracts.constants import attribute, saveable
...
@dataclass
class HoldsPrimitives(Saveable):
...
@dataclass
class HoldsNestedData(Saveable):
    str_: str = "0"
    int_: int = 0
    lst_: list[str] = field(default_factory=list)
...
@dataclass
class MyMixedSaveable(Saveable):
    str_: str = "0"
    lst_: list[str] = field(default_factory=list)
...
meta_saveable = MetaData(
    ...
    role=attribute,
    element_type=python_type_literal_map[type(None)],  # type: ignore[arg-type]
)
...
meta_str = MetaData(
    ...
    role=attribute,
    element_type=python_type_literal_map[str],  # type: ignore[arg-type]
)
...
meta_int = MetaData(
    ...
    role=attribute,
    element_type=python_type_literal_map[int],  # type: ignore[arg-type]
)
...
def create_saveable_from_datafields(name: str, data_fields: tuple[DataField]) -> type:
    ...
    for data_field in data_fields:
        ...
        if (
            ...
            fields.append((data_field.meta.name, Any, field(default_factory=factory)))  # type: ignore[arg-type, call-arg] # noqa: E501
        else:
            factory = python_type_literal_map_reversed[python_type_]  # type: ignore[assignment] # noqa: E501
            fields.append((data_field.meta.name, Any, field(default_factory=factory)))  # type: ignore[arg-type, call-arg] # noqa: E501
