...
# a list / tuple / set / Saveable, its default values
# MUST BE AN EMPTY list / tuple / set / Saveable.
...
path = Path(__file__).parent / "person.xml"
if path.exists():
    Path.unlink(path)
if not path.exists():
...
# method save
with XmlFile(path, write_mode) as f:
    f.save(person)
...
# call method load
with XmlFile(path, read_mode) as f:
